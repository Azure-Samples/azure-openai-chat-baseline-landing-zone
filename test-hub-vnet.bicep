...
resource hubVirtualNetwork 'Microsoft.Network/virtualNetworks@2024-05-01' = {
  ...
  properties: {
    ...
    // DNS servers will be configured after DNS Resolver is deployed
    subnets: [
      {
          ...
          rules: [
            {
              ...
              ipProtocols: ['TCP', 'UDP']
              sourceAddresses: ['10.0.0.0/16', '192.168.0.0/16']
...
resource bastionPublicIp 'Microsoft.Network/publicIPAddresses@2024-05-01' = {
  ...
  properties: {
    ...
    deleteOption: 'Delete'
    dnsSettings: { domainNameLabel: 'bastion-${hubBaseName}' }
    publicIPAddressVersion: 'IPv4'
    publicIPAllocationMethod: 'Static'
  }
}
...
// Update VNet with DNS Resolver settings after DNS Resolver is deployed
resource updateVNetWithDnsSettings 'Microsoft.Resources/deployments@2024-03-01' = {
  name: 'updateVNetDns'
  properties: {
    mode: 'Incremental'
    template: {
      ...
      resources: [
        {
          ...
          properties: {
            addressSpace: { addressPrefixes: [hubVirtualNetworkAddressPrefix] }
            dhcpOptions: { dnsServers: ['10.0.3.4'] }
            subnets: [
              {
                name: 'AzureFirewallSubnet'
                properties: {
...
// DNS Forwarding Rules for Private DNS Zones
